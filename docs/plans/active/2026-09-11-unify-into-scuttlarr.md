...
- [x] 1.3 (existing `hooks::boot` stale-path repair; signatures are file-name based) Hook adapter: `hooks/claude-status.py` installs to the new path; `SIGNATURES` in
...
- [x] 1.4 (`desktop.rs` `is_generated`) Files we generate outside our config (`aerospace.toml` marker
