...
>
> Last updated: 2026-09-11 (unification into scuttlarr: phase 1 rename landed in code)

## Where we are

**Unification into scuttlarr, 2026-09-11 (phase 0 done, phase 1 code landed):** launcharr
and scuttlarr fold into this repo, renamed scuttlarr; the launcher is a feature (DECISIONS
2026-09-11, plan `plans/active/2026-09-11-unify-into-scuttlarr.md`). Code rename done in
one pass: `@scuttlarr/*` scopes, crate `scuttlarr`/`scuttlarr_lib`, `productName` +
`com.mitchmalone.scuttlarr`, every owned home path, env `SCUTTLARR_PLUGIN`, protocol
`scuttlarr-plugin://`, default theme `scuttlarr` (`launcharr` aliased in `resolveTheme`).
`rename.rs` moves `~/.config`, `~/.local/{share,state}`, Application Support, purges the
plugin build cache, drops the old LaunchAgent; `desktop.rs` still reads
`# generated by launcharr` as ours; hook paths in `~/.claude*/settings.json` mend through
the existing stale-path repair. `pnpm verify` green (215 Rust tests). **Not pushed.**
Outward-facing steps are Mitch's: rename the GitHub repo (1.1), add `Casks/scuttlarr.rb`

- deprecate `launcharr.rb` in the tap (1.5), point scuttlarr.com at this `apps/www` and
  301 launcharr.com (1.6) — until then `site.ts`, README, and the workflow name a repo, cask,
  and domain that don't exist yet. Then 1.7 the release. Phase 2 (port `packages/setup`)
  after.
